...
public class GreenGhost : MonoBehaviour {
	...
	private Vector2 RealSTarget = new Vector2(3.589713f, -2.539198f), STarget, CTarget, position2D;
	...
	private GameObject player, mtimer, red, rstrt;
	private char mode;
	...
	private int randdir;
	private int steps = 3;                                     //used for entering/exiting ghost house
	private Sprite scaredGhost, eatenGhost, greenGhost;
	...
	void Start () {
		...
		greenGhost = Resources.Load("greenghost", typeof(Sprite)) as Sprite;
	...
	void Update () {
	//do nothing if ghost can't exit or is currently exiting/entering ghost house
	if (canExit) {
	if (steps != 6) {
	//NORMAL MODE--------------------------------------------------------------------------
		...
		mode = mtimer.GetComponent<Modetimer>().mode;                                //is it scare mode
		if (mode == 's') {
			scare = true;
			CancelInvoke();
	...
	if (noteaten) {
	//SCARE MODE--------------------------------------------------------------------------------------------
		if (mtimer.GetComponent<Modetimer>().mode != 's') {               //reset if scare ended
			scare = false;
			CancelInvoke();
			...
			STarget = RealSTarget;
			this.GetComponent<SpriteRenderer>().sprite = greenGhost;
			InvokeRepeating("Movement", 0.1f, 0.22f);
		}
	} else {
	//EATEN MODE---------------------------------------------------------------------------------------------
		if (justEaten) {                   //set up eaten mode
			CancelInvoke();
		...
		position2D.x = this.transform.position.x;
		position2D.y = this.transform.position.y;
		hit = Physics2D.Raycast(position2D, Vector2.right, 0.05f, IntersectionLayerMask);      //catch intersections
		if (hit.collider != null) {
			changeDir = true;
			inters = true;
		} else {inters = false;}
		...
		if (hit.collider != null) {
			if (hit.collider.gameObject.tag == "Respawn") {
				...
				InvokeRepeating("Movement", 0.05f, 0.3f);
				steps = 0;
				mtimer.GetComponent<Modetimer>().greenexit = false;
			}
		} 
	}
	}
		...
		if ((changeDir) && (modeChange == false)) {
			if (inters == false) {
				...
				if (chase == false) {
					...
					CTarget = new Vector2(player.transform.position.x, player.transform.position.y);   //Green's target is first finding a dot two tiles in direction of pacman(left overflow bug)
					if (player.GetComponent<Playerscript>().direct == 'L') { CTarget.x -= 0.6f; }      //and then finding a symmetrical tile of Red Ghost's in accordance to that dot
					if (player.GetComponent<Playerscript>().direct == 'R') { CTarget.x += 0.6f; }
					if (player.GetComponent<Playerscript>().direct == 'D') { CTarget.y -= 0.6f; }
					if (player.GetComponent<Playerscript>().direct == 'U') { CTarget.x -= 0.6f; CTarget.y += 0.6f; }
					if (CTarget.x < red.transform.position.x) {
						CTarget.x = CTarget.x - (red.transform.position.x - CTarget.x);
					} else {
						CTarget.x = CTarget.x + (CTarget.x - red.transform.position.x);
					}
					if (CTarget.y < red.transform.position.y) {
						CTarget.y = CTarget.y - (red.transform.position.y - CTarget.y);
					} else {
						CTarget.y = CTarget.y + (CTarget.y - red.transform.position.y);
					}
					PickDirC();
				}
			}
	...
	public void Restart() {
		position.x = -0.3103402f;
		position.y = 0.4607985f;
		position.z = -1;
		this.transform.position = position;
		position2D.x = this.transform.position.x;
		position2D.y = this.transform.position.y;
